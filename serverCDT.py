...
def base_json():
    ...
    return cache_response(resp, max_age=86400)

@app.route("/delta.json")
def delta_json():
    resp = make_response(send_from_directory('.', 'delta.json'))
    resp.headers['Content-Type'] = 'application/json'
    resp.headers['Use-As-Dictionary'] = 'match="/base.json"; match-dest=fetch'
    return cache_response(resp, max_age=3600)
...
#     resp = Response(compressed_data, mimetype='application/octet-stream')
